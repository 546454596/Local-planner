...
void LocalPlannerNode::Loop() {
  roborts_common::ErrorInfo error_info = local_planner_->Initialize(local_cost_, tf_, visual_);//指向 TEB类，调用它的Initialize函数
  if (error_info.IsOK()) {
...
void LocalPlannerNode::StartPlanning() {
  ...
  local_planner_thread_= std::thread(std::bind(&LocalPlannerNode::Loop,this));//子线程
}
